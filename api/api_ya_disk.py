...
def file_url_upload(path: str, file_name: str, headers: dict[str, str], overwrite=None) -> str:
    ...
    base_url: str = f'https://cloud-api.yandex.net/v1/disk/resources/upload?path={path}/{file_name}'
    response: Response = requests.get(base_url, headers=headers, params=params, timeout=10)
...
class ApiYandexDisk:
    ...
    def get_info(self, path) -> dict[str, float]:
        ...
        params: dict[str, str] = {'fields': '_embedded.items.name,_embedded.items.modified'}
        response: Response = requests.get(base_url, headers=self.headers, params=params, timeout=10)
        ...
        try:
            json_response: dict = response.json()
            result: dict[str, float] = {elem['name']: datetime.fromisoformat(elem['modified']).timestamp()
